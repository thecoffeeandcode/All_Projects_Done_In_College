<?php
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT med_isbn, med_image FROM meds ORDER BY med_isbn DESC";
	...
	function getBookByIsbn($conn, $isbn){
		$query = "SELECT med_title, med_dom, med_price FROM meds WHERE med_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getbookprice($isbn){
		$conn = db_connect();
		$query = "SELECT med_price FROM meds WHERE med_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getPubName($conn, $pubid){
		...
		if(mysqli_num_rows($result) == 0){
			echo "Empty meds ! Something wrong! check again";
			exit;
		}
	...
	function getAll($conn){
		$query = "SELECT * from meds ORDER BY med_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
